...
function handleOrgJoinOrCreatePolling({ isJoiningOrg, orgName, contactEmail, originalOrgId }) {
  ...
  async function checkOrgIdChanged() {
    attempts++;
    try {
      const response = await apiClient.get(`${PROJECT_URL}/api/dm/getData?token=your_token_here&dm_name=ss_user&max_order=0&format=json&mode=download&where=\`username\`="${USERNAME}"`, {}, true);
  ...
  if (!foundOrg) {
    ...
    try {
      const response = await apiClient.post(
        'https://es.rta.vn/nerp_org/_search',
        {
          "size": 1,
    ...
    try {
      ...
      await apiClient.post(
        'https://automation.rta.vn/webhook/rthrm-events',
        payload,
        { 'Content-Type': 'application/json' },
        false
      );
...
async function fetchData(targetYear = null, targetMonth = null) {
  ...
  try {
    ...
    let year, month, dateRange;
    if (targetYear !== null && targetMonth !== null) {
      year = targetYear;
      month = targetMonth;
      dateRange = DateRangeCalculator.getMonth(year, month);
    } else {
      dateRange = DateRangeCalculator.getCurrentMonth();
      year = dateRange.year;
      month = dateRange.month;
    }

    // Check cache first
    const cachedAttendance = hrDataCache.get('attendance', year, month);
    const cachedLeave = hrDataCache.get('leave', year, month);
    const cachedHoliday = hrDataCache.get('holiday', year, month);

    let attendanceResult, leaveResult, holidayResult, latestCheckinResult;

    if (cachedAttendance && cachedLeave && cachedHoliday) {
      // Use cached data
      console.log(`[HR Optimization] Using cached data for ${year}-${month}`);
      attendanceData = cachedAttendance;
      leaveData = cachedLeave;
      holidayData = cachedHoliday;

      // Still fetch latest check-in (always fresh)
      latestCheckinResult = await apiClient.post(
        'https://es.rta.vn/hr_checkinout_list_v4/_search',
        HRQueryBuilder.buildLatestAttendanceQuery(USER_ORG_ID, USERNAME, 1),
        { 'Content-Type': 'application/json' },
        true
      );
    } else {
      // Fetch from API
      console.log(`[HR Optimization] Fetching data for ${year}-${month} (${dateRange.start} to ${dateRange.end})`);

      [attendanceResult, leaveResult, holidayResult, latestCheckinResult] = await Promise.all([
        apiClient.post(
          'https://es.rta.vn/hr_checkinout_list_v4/_search',
          HRQueryBuilder.buildAttendanceQuery(USER_ORG_ID, USERNAME, dateRange.start, dateRange.end),
          { 'Content-Type': 'application/json' },
          true
        ),
        apiClient.post(
          'https://es.rta.vn/hr_leave_tracking/_search',
          HRQueryBuilder.buildLeaveQuery(USER_ORG_ID, USERNAME, dateRange.start, dateRange.end),
          { 'Content-Type': 'application/json' },
          true
        ),
        apiClient.post(
          'https://es.rta.vn/erp_holiday/_search',
          HRQueryBuilder.buildHolidayQuery(USER_ORG_ID, dateRange.start, dateRange.end),
          { 'Content-Type': 'application/json' },
          true
        ),
        apiClient.post(
      ...
      attendanceData = attendanceResult.data?.hits ? attendanceResult.data.hits.hits.map(hit => hit._source) : [];
      leaveData = leaveResult.data?.hits ? leaveResult.data.hits.hits.map(hit => hit._source) : [];
      holidayData = holidayResult.data?.hits ? holidayResult.data.hits.hits.map(hit => hit._source) : [];

      // Cache the fetched data
      hrDataCache.set('attendance', year, month, attendanceData);
      hrDataCache.set('leave', year, month, leaveData);
      hrDataCache.set('holiday', year, month, holidayData);
    }
    ...
    console.log(`[HR Optimization] Load time: ${loadTime}ms | Records: Attendance=${attendanceData.length}, Leave=${leaveData.length}, Holiday=${holidayData.length}`);
    console.log(`[HR Optimization] Cache stats:`, hrDataCache.getStats());
    ...
    errorElem.style.display = 'block';
    console.error('[HR Optimization] Fetch error:', error);
  }
}
...
function goToPrevMonth() {
  ...
  // Fetch data for the new month (with cache check)
...
function goToPrevWeek() {
  ...
  const newMonth = weekStartDate.getMonth();
  const newYear = weekStartDate.getFullYear();
  ...
  if (newMonth !== currentMonth || newYear !== currentYear) {
    ...
    // Same month, just update the view
    updateWeekTitle();
    renderWeekView();
  }
}
...
function autoRefreshAll() {
  // Clear cache on auto-refresh to ensure fresh data
  hrDataCache.clear();
  console.log('[HR Optimization] Auto-refresh triggered, cache cleared');
  fetchData();
}
