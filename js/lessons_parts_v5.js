...
function playWarmupAudio(audioPath, button) {
    console.log('🎵 playWarmupAudio called:', {
        audioPath,
        isPlaying,
    ...
    // Toggle: Nếu đang play thì stop, nếu đang stop thì play
    if (isPlaying && currentAudio) {
        ...
        console.log('✅ Audio stopped, isPlaying:', isPlaying);
    } else {
        console.log('▶️ Starting new audio...');
        stopCurrentAudio();
        resetAllAudioButtonsUI();
        
        // Set button to stop state before playing
        if (button) {
            const icon = button.querySelector('i');
            if (icon) {
                icon.className = 'fas fa-stop text-white text-sm md:text-xl group-hover:scale-110 transition-transform duration-200';
            }
            button.classList.add('animate-pulse-ring');
        }
        
        playAudioFile(audioPath, button);
        console.log('🎶 playAudioFile called, isPlaying:', isPlaying);
    }
}
...
function playModalAudio() {
    ...
    if (currentActivity && currentActivity.type === 'warmup') {
        ...
        if (isPlaying && currentAudio) {
            ...
            playAudioFile(safeGetAssetUrl(currentActivity.content.audio), button);
        }
    }
}
...
function renderWarmupActivity(activity) {
    ...
    if (modalDescriptionText && activity.content.description) {
        modalDescriptionText.textContent = activity.content.description;
        console.log('📝 Description set for modal:', activity.content.description);
    } else {
        console.log('⚠️ Modal description element not found or no description content:', {
            ...
            description: activity.content.description || 'no description'
        });
    }
}
