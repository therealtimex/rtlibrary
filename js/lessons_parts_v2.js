...
async function toggleRecording() {
    // Prioritize AppFile interface if available
    if (typeof AppFile !== 'undefined' && AppFile.recordAudio) {
        updatePronunciationStatus("Starting native audio recorder...", "info");
        AppFile.recordAudio("audioRecordedCallback");
        return;
    }

    // Fallback to web-based recording if AppFile is not available
    if (!audioContext) {
...
// Callback function for AppFile.recordAudio
function audioRecordedCallback(result) {
    if (result.error) {
        console.error("AppFile recording error:", result.error);
        updatePronunciationStatus(`Recording failed: ${result.error}`, "error");
        return;
    }

    console.log("Audio recorded via AppFile:", result);
    updatePronunciationStatus(`Recording complete: ${result.name}. Processing...`, "info");

    // Use fetch to get the blob from the native URI
    fetch(result.uri)
        .then(response => {
            if (!response.ok) {
                throw new Error(`Failed to fetch recorded audio. Status: ${response.statusText}`);
            }
            return response.blob();
        })
        .then(blob => {
            // Determine audio format from MIME type
            const audioFormat = getFormatFromMimeType(result.type);

            // We have the blob, now convert to Base64 and analyze
            const reader = new FileReader();
            reader.onload = () => {
                const base64Audio = reader.result.split(',')[1];
                analyzeAudio(base64Audio, audioFormat); // Pass base64 and format
            };
            reader.onerror = (error) => {
                throw new Error("Failed to read audio blob as Base64: " + error);
            };
            reader.readAsDataURL(blob);

            // Also, display the audio for playback
        ...
        .catch(error => {
            ...
            updatePronunciationStatus(`Error processing audio: ${error.message}`, "error");
        });
}
...
function getFormatFromMimeType(mimeType) {
    ...
    // Handle specific cases like 'x-m4a' -> 'm4a'
...
// Helper to update UI status in pronunciation activity
function updatePronunciationStatus(message, type) {
    const recordStatus = document.getElementById('record-status');
    if (recordStatus) {
        let icon = '';
        let colorClass = '';
        ...
        recordStatus.innerHTML = `<span class="${colorClass} font-semibold">${icon} ${message}</span>`;
    }
}
...
async function analyzeAudio(base64Audio, audioFormat = 'wav') { // Default to 'wav' for backward compatibility
    ...
    // If base64Audio is not provided, get it from the recordedBlob (web fallback)
    if (!base64Audio) {
        if (!recordedBlob) {
        ...
        try {
            ...
            alert('Could not process recorded audio. Please try again.');
            if (loading) loading.classList.add('hidden');
            return;
        }
    }
    ...
    try {
        ...
        const response = await fetch('https://hooks.realtimex.co/hooks/anki-speech-v2', {
            ...
            body: JSON.stringify({
                ...
                audio_format: audioFormat // Use the detected format
            })
        });
