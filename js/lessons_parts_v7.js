...
function startVoiceActivityDetection() {
    ...
    // Mobile fallback: Skip complex voice detection on mobile devices
    ...
    // Desktop: Use advanced voice detection
        ...
        currentAudio.onended = () => {
            ...
            if (button && button.querySelector('i')) {
                ...
                const isWarmupButton = button.closest('.animate-slide-in') ||
                    icon.className.includes('ml-0.5');
                if (isWarmupButton) {
...
function toggleAudioPlayback(audioPath, button) {
    ...
    if (isPlaying && currentAudio) {
        ...
        // playAudioFile internally calls stopCurrentAudio and resetAllAudioButtonsUI,
...
function playWarmupAudio(audioPath, button) {
    ...
    // The complex UI logic is now handled by playAudioFile and resetAllAudioButtonsUI
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('keydown', function (e) {
        ...
        // Close pronunciation modal on ESC key
        if (e.key === 'Escape') {
            const modal = document.getElementById('pronunciation-results-modal');
...
function showActivity(index) {
    ...
    if (activity.type !== 'pronunciation') {
        if (renderPronunciationActivity.lastActivityIndex !== undefined) {
            debugLog(`Resetting pronunciation activity flag when switching from pronunciation to ${activity.type}`);
            delete renderPronunciationActivity.lastActivityIndex;
        }
    }
...
function savePronunciationState(data) {
    ...
    debugLog('Saving pronunciation state');
    debugLog('currentActivityIndex:', currentActivityIndex);
    debugLog('currentWordIndex:', currentWordIndex);
    debugLog('wordId:', wordId);
    debugLog('data keys:', Object.keys(data));
    debugLog('Total pronunciation states after save:', Object.keys(pronunciationState).length);
    debugLog('All state keys:', Object.keys(pronunciationState));
}
...
// Restore pronunciation state with explicit indices (to avoid timing issues)
function restorePronunciationStateWithIndices(activityIndex, wordIndex) {
    ...
    debugLog('restorePronunciationStateWithIndices called');
    debugLog('activityIndex:', activityIndex);
    debugLog('wordIndex:', wordIndex);
    debugLog('wordId:', wordId);
    debugLog('Total pronunciation states:', Object.keys(pronunciationState).length);
    debugLog('Available pronunciation states:', Object.keys(pronunciationState));
    debugLog('WordState exists:', !!wordState);
    debugLog('Looking for wordId:', wordId, 'in states:', Object.keys(pronunciationState));

    if (!wordState) {
        debugLog('No wordState found for:', wordId);
        // Still need to hide View Results button if no state
        const viewResultsBtn = document.getElementById('view-results-btn');
        if (viewResultsBtn) {
            viewResultsBtn.classList.add('hidden');
            debugLog('View Results button hidden (no state)');
        }
        return;
    }
    ...
    if (wordState.hasAnalysis) {
        ...
        debugLog('Restoring analysis results for word:', wordId);

        // Populate inline results for state persistence but keep hidden
        if (resultsSection && resultsContent) {
            resultsContent.innerHTML = wordState.analysisResult;
            // Keep results section hidden - only use popup modal
            resultsSection.classList.add('hidden');
            debugLog('Inline results populated but kept hidden');
        }

        // Show View Results button if analysis exists
        if (viewResultsBtn) {
            viewResultsBtn.classList.remove('hidden');
            debugLog('View Results button restored for word:', wordId);
        } else {
            // Retry after a short delay if button not found
            setTimeout(() => {
                const retryBtn = document.getElementById('view-results-btn');
                if (retryBtn) {
                    retryBtn.classList.remove('hidden');
                    debugLog('View Results button restored (retry) for word:', wordId);
                } else {
                    debugLog('View Results button still not found after retry for word:', wordId);
                }
            }, 50);
        }
    } else {
        debugLog('No analysis results found for word:', wordId);
        // Hide View Results button if no analysis exists
        const viewResultsBtn = document.getElementById('view-results-btn');
        if (viewResultsBtn) {
            viewResultsBtn.classList.add('hidden');
            debugLog('View Results button hidden (no analysis)');
        }
    }
}
...
function showStoredResults() {
    ...
    if (wordState && wordState.analysisResult) {
        ...
        debugLog('Showing stored pronunciation results for word:', wordId);
    } else {
        debugLog('No stored results found for word:', wordId);
    }
}
...
function renderPronunciationActivity(container, content) {
    // Only reset currentWordIndex if this is the first time rendering this activity
    // (not when navigating between words within the same activity)
    ...
    const renderPronunciation = () => {
            ...
            <div class="animate-slide-in">
                <div class="bg-white rounded-3xl shadow-lg mb-4 overflow-hidden">
                    ...
                    <div class="p-4 sm:p-6">
                        ...
                        <!-- Progress Dots -->
                        ${createProgressDots({
            count: content.practice_words.length,
            currentIndex: currentWordIndex,
            studiedItems: studiedWords,
            theme: 'purple'
        })}

                        <!-- Navigation -->
                        ${createNavigationButtons({
            prevAction: 'prevPronunciation()',
            nextAction: 'nextPronunciation()',
            isFirst: currentWordIndex === 0,
            isLast: currentWordIndex === content.practice_words.length - 1,
            theme: 'purple',
        ...
        // Use setTimeout to ensure DOM is fully rendered before restoring state
        setTimeout(() => {
            // Pass current indices explicitly to avoid timing issues
            const activityIndex = currentActivityIndex;
            const wordIndex = currentWordIndex;
            debugLog('Calling restorePronunciationState with explicit indices:', activityIndex, wordIndex);
            restorePronunciationStateWithIndices(activityIndex, wordIndex);
        }, 10);
    };
...
function restorePronunciationState() {
    ...
    debugLog('restorePronunciationState called');
    debugLog('currentActivityIndex:', currentActivityIndex);
    debugLog('currentWordIndex:', currentWordIndex);
    debugLog('wordId:', wordId);
    debugLog('Available pronunciation states:', Object.keys(pronunciationState));
    ...
    if (wordState.analysisResult) {
        ...
        debugLog('Restoring analysis results for word:', wordId, 'Has results:', !!wordState.analysisResult);
        ...
        if (viewResultsBtn) {
            viewResultsBtn.classList.add('hidden');
            debugLog('View Results button hidden for word:', wordId);
        }
    }
}
...
// ===== PHASE 2: Enhanced Progress Bar with Icons =====
...
const activityTypeIcons = {
    ...
    'quiz': { name: 'Quiz', icon: 'fas fa-question-circle', color: 'text-indigo-500' },
    'congratulations': { name: 'Complete', icon: 'fas fa-trophy', color: 'text-yellow-500' }
};


// ===== PHASE 1: Data Restructure & Enhanced Navigation =====
...
function safeGetAssetUrl(path) {
    ...
    if (path.startsWith('http')) return path;
    const cleanPath = path.startsWith('/') ? path.substring(1) : path;
    return `https://app.realtimex.co/storage/v1/object/public/app_lang_assets_data/${cleanPath}`;
}

// Override loadLessonData to restructure data
const originalLoadLessonData = window.loadLessonData || loadLessonData;


// Override showActivity to handle new activity types
const originalShowActivity = window.showActivity || showActivity;
if (typeof originalShowActivity === 'function') {
    window.showActivity = function (index) {
        ...
        console.log('Showing activity:', { index, type: activity.type });

        // Handle different activity types
        if (activity.type === 'warmup') {
            const contentDiv = document.getElementById('lesson-content');
            renderWarmupActivity(contentDiv, activity.content);
        } else if (activity.type === 'congratulations') {
            const contentDiv = document.getElementById('lesson-content');
            renderCongratulationsActivity(contentDiv, activity.content);
        } else {
            // Use original rendering for other activities
            const result = originalShowActivity.call(this, index);
        }
...
function renderProgressBar() {
    ...
    learningActivities.forEach((activity, index) => {
        ...
        if (isMainActivity) {
            ...
            label.innerHTML = `
                <div class="text-xs">${activityInfo.name}</div>
            `;
        } else {
            ...
            label.innerHTML = `
                <div class="hidden sm:block text-xs opacity-75">${activityInfo.name}</div>
            `;
        }
