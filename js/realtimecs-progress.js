        ...
        const queryBody = {
            ...
            "query": {
                "bool": {
                    ...
                    "must_not": [{
                            "terms": {
                                "output.data_formatted.current_status.keyword": ["Closed", "Close", "close", "closed", "Đóng", "đóng", "Customer Replied", "Sent SMS", "Sent Email", "Sent Invoice", "Send Invoice to Customer"]
                            }
                        },
                        {
                            "terms": {
...
function populateRepairData(hits) {
    ...
    document.getElementById('customer-name').textContent = maskName(initialOrderData.customer_name || '');
    document.getElementById('customer-phone').textContent = maskPhone(initialOrderData.customer_phone || '');
    document.getElementById('customer-email').textContent = maskEmail(initialOrderData.customer_email || '');

    // Update device information
    document.getElementById('device-brand').textContent = initialOrderData.device_brand || '';
    document.getElementById('device-model').textContent = initialOrderData.device_model || '';
    document.getElementById('device-type').textContent = capitalizeFirst(initialOrderData.device_type) || '';
