...
// Helper function to make secure Elasticsearch API calls using App.callApi
function callElasticsearchApi(url, body, callbackName) {
	return callSecureApi(url, body, callbackName);
}
...
async function fetchData() {
	...
	try {
		const [attendanceResult, leaveResult, holidayResult] = await Promise.all([
			...
			callElasticsearchApi('https://es.rta.vn/hr_leave_tracking/_search', {
				"size": 10000,
				"collapse": { "field": "keyid_ins.raw" },
			...
			callElasticsearchApi('https://es.rta.vn/erp_holiday/_search', {
				"size": 10000,
				"collapse": { "field": "keyid_ins.raw" },
...
async function fetchDataNotif() {
	const data = await callElasticsearchApi('https://es.rta.vn/hrm_notif/_search', {
		"size": 10000,
