...
// Consolidated Progress Bar - handles all activity types with enhanced UI
function renderProgressBar() {
    ...
    // Activity type mapping with icons (fallback if not defined elsewhere)
    const activityTypeIcons = window.activityTypeIcons || {
        ...
        'pronunciation': { name: 'Pronunciation', icon: 'fas fa-microphone', color: 'text-purple-500' },
        'dialog': { name: 'Dialog', icon: 'fas fa-comments', color: 'text-green-500' },
        'quiz': { name: 'Quiz', icon: 'fas fa-question-circle', color: 'text-orange-500' },
    ...
    learningActivities.forEach((activity, index) => {
        ...
        if (isWarmup || isCongratulations) {
            // STANDALONE: Warmup and Congratulations - no connecting bars, just icons
            stepContainer.className = 'flex items-center justify-center px-4';
        } else {
            // CONNECTED: Main activities (vocab, pronunciation, dialog, quiz) with progress bars
        ...
        if (isWarmup || isCongratulations) {
            // STANDALONE indicators: larger, no positioning on bars
            ...
            // CONNECTED indicators: positioned on progress bars
        ...
        // Set indicator state and content
        if (index < currentActivityIndex) {
        ...
        if (isMainActivity) {
            // Main activities: always show labels (core learning activities)
            ...
            label.innerHTML = `
                <div class="flex items-center justify-center">
                    <i class="${activityInfo.icon} ${activityInfo.color} text-sm"></i>
                </div>
                <div class="text-xs">${activityInfo.name}</div>
            `;
        } else {
            // Milestone activities: minimal labels, hidden on mobile
...
if (typeof originalLoadLessonData === 'function') {
    window.loadLessonData = function (jsonData) {
        try {
            // Sort original data
            const sortedData = jsonData.sort((a, b) => a.order - b.order);

            // Extract warmup and learning activities
            originalWarmupData = sortedData.find(item => item.type === 'warmup');
            originalLearningActivities = sortedData.filter(item => item.type !== 'warmup');
            ...
            console.error('Error restructuring lesson data:', error);
            // Fallback to original function
            return originalLoadLessonData.call(this, jsonData);
        }
    };
}
...
if (typeof originalShowActivity === 'function') {
    window.showActivity = function (index) {
        ...
        if (activity.type === 'warmup') {
            renderWarmupActivity(activity);
        } else if (activity.type === 'congratulations') {
            renderCongratulationsActivity(activity);
        } else {
...
// New render functions for warmup and congratulations
function renderWarmupActivity(activity) {
    const contentDiv = document.getElementById('lesson-content');
    const avatarUrl = safeGetAssetUrl(activity.content.avatar);
    const audioUrl = safeGetAssetUrl(activity.content.audio);
            ...
            <div class="bg-white rounded-3xl shadow-lg overflow-hidden">
                <div class="relative w-full h-72 md:h-80 overflow-hidden group">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent flex items-center justify-center">
                        <div class="flex gap-3 md:gap-6 items-center transform hover:scale-105 transition-transform duration-300">
                            <button onclick="playAudioFile('${audioUrl}', this)" title="Listen to description" 
                ...
                <div class="p-6 text-center">
                    ...
                    <p class="text-gray-600 mb-6">Let's start your learning journey. Click the buttons above to explore the lesson content.</p>
                    <div class="text-sm text-gray-500">Activity 1 of ${learningActivities.length}</div>
                </div>
            </div>
        </div>
    `;
...
function renderCongratulationsActivity(activity) {
    const contentDiv = document.getElementById('lesson-content');
    contentDiv.innerHTML = `
        <div class="animate-slide-in">
            <div class="bg-white rounded-3xl shadow-lg p-8 text-center completion-screen">
                <div class="text-6xl mb-6">🎉</div>
                <h2 class="text-3xl font-bold text-gray-800 mb-4">${activity.content.title}</h2>
                <p class="text-lg text-gray-600 mb-8">${activity.content.message}</p>
...
// ===== ENHANCED PROGRESS BAR - NO CONNECTION BETWEEN WARMUP AND VOCAB =====

// Enhanced renderProgressBar function - warmup standalone, no connection to vocab
function renderProgressBar() {
    const progressContainer = document.getElementById('activity-progress');
    const labelsContainer = document.getElementById('progress-labels');
    ...
    learningActivities.forEach((activity, index) => {
        ...
        if (isWarmup || isCongratulations) {
            // Milestone activities: standalone icons, no connecting bars
            stepContainer.className = 'flex items-center justify-center px-2';
        } else {
            // Main activities (vocab, pronunciation, dialog, quiz): connected with progress bars
            stepContainer.className = 'flex-1 h-1.5 bg-gray-200 rounded-full relative transition-all duration-300 cursor-pointer hover:bg-gray-300';
        ...
        if (isWarmup || isCongratulations) {
            // Milestone indicators: larger, standalone
            indicator.className = 'w-8 h-8 rounded-full flex items-center justify-center text-sm transition-all duration-300 border-2 border-white shadow-lg';
        } else {
            // Main activity indicators: positioned on progress bar
            indicator.className = 'absolute -top-3 -right-1 w-7 h-7 rounded-full flex items-center justify-center text-xs transition-all duration-300 border-2 border-white shadow-lg';
        }
        ...
        // Create labels - optimized for mobile
        const label = document.createElement('div');

        if (isMainActivity) {
            // Main activities: always show labels (these are the core learning activities)
            label.className = 'text-xs text-center min-w-[60px] cursor-pointer p-1 rounded-md transition-all duration-300 hover:bg-gray-100 flex flex-col items-center gap-1';
            ...
            // Milestone activities: minimal labels, hidden on mobile for cleaner look
            label.className = 'text-xs text-center min-w-[40px] cursor-pointer p-1 transition-all duration-300 flex flex-col items-center gap-1';
...
// Override the original renderProgressBar function
window.renderProgressBar = renderProgressBar;
// ===== PROGRESS BAR WITH STANDALONE WARMUP & CONGRATULATIONS =====


// Enhanced renderProgressBar - warmup and congratulations are standalone
function renderProgressBar() {
    const progressContainer = document.getElementById('activity-progress');
    const labelsContainer = document.getElementById('progress-labels');
...
// Override the global renderProgressBar function
window.renderProgressBar = renderProgressBar;